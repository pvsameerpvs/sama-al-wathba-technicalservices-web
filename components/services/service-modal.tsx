...
export function ServiceModal({ service, onClose }: ServiceModalProps) {
  ...
  return (
    <div
      ...
      onClick={onClose}
    >
      {/* Wrapper to center and add padding on mobile */}
      <div
        className="flex min-h-full items-center justify-center px-3 sm:px-4 py-6"
        onClick={(e) => e.stopPropagation()}
      >
        <div className="relative max-w-3xl w-full rounded-3xl bg-white shadow-xl overflow-hidden flex flex-col max-h-[90vh]">
          {/* Top image */}
          <div className="relative h-56 sm:h-64 md:h-72 w-full overflow-hidden">
            ...
            <button
              type="button"
              onClick={onClose}
              className="absolute right-4 top-4 inline-flex h-9 w-9 items-center justify-center rounded-full bg-white/90 text-slate-700 shadow hover:bg-white"
              aria-label="Close"
            >
              <X className="h-4 w-4" />
            </button>
                    ...
                    <ul className="space-y-1.5 text-sm text-slate-700">
                      {service.highlights.map((item) => (
                        <li key={item} className="flex gap-2">
                          <span className="mt-1 h-1.5 w-1.5 rounded-full bg-[#0054A6]" />
                          <span>{item}</span>
                        </li>
                      ))}
                    </ul>
