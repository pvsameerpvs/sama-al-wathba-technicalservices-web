...
export function HeroSlider() {
  ...
  return (
    <section className="relative h-screen w-full overflow-hidden bg-slate-950 text-slate-50 flex items-center">
      {/* Background image */}
      ...
      {/* Content Wrapper */}
      <div className="relative z-10 w-full max-w-7xl mx-auto px-4 flex flex-col gap-12 lg:flex-row lg:items-center">
        {/* Left Side Text */}
        <div className="max-w-xl space-y-6">
          <p className="inline-flex items-center rounded-full bg-white/10 px-3 py-1 text-xs font-medium uppercase tracking-[0.25em] text-brand-yellow ring-1 ring-white/15 backdrop-blur">
        ...
        <div className="relative w-full max-w-md ml-auto rounded-2xl bg-white/5 p-4 ring-1 ring-white/10 backdrop-blur">
          <div className="mb-4 flex items-center justify-between text-xs uppercase tracking-[0.25em] text-slate-300">
            <span>{slide.title}</span>
            <span>{String(active + 1).padStart(2, "0")}</span>
          </div>

          <div className="relative h-64 overflow-hidden rounded-xl border border-white/15">
            <Image
              src={slide.image}
              alt={slide.title}
              fill
          ...
          <p className="mt-4 text-sm text-slate-200">{slide.subtitle}</p>
            ...
            {slides.map((_, i) => (
              <button
                ...
                className={`h-2 w-6 rounded-full transition-all ${
                  i === active ? "bg-brand-yellow" : "bg-white/25"
                }`}
              />
            ))}
          </div>
